...
**More bundled texts still unparsed**: `sources/shennubencao.txt` contains not just 神农本草经 and 难经 (added 2026-07-19) but three more classics stacked after 难经 ends — 《辅行诀脏腑用药法要》(~1,370 lines), 《金匮要略》相关讨论 (~1,335 lines, may be commentary rather than primary text — check before treating as translation-worthy source), and 《奇经八脉考》(~376 lines, structured per extraordinary vessel). Same "extra text with no clean separator" pattern as everything else in this file; none of the three has been parsed yet. `难经`'s numbering was genuinely tricky (see `build_nanjing.py`'s docstring for the local-vs-global counter issue) — expect the next one to have its own quirks, don't assume `build_data.py`'s entry-number-is-authoritative assumption holds.
