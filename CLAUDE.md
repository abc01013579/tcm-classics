...
**随笔 (journal) bilingual support (added 2026-07-20)**: unlike the other classics, journal entries are single self-contained Markdown files, so bilingual pairing lives inside the same `.md` file rather than a separate `build_*_en.py` + batch pipeline. An `<!--en-->` marker on its own line splits the body into zh/en halves; `build_journal.py` splits each half into paragraphs (blank-line-delimited) and zips them 1:1 by position into `pairs` — same "paragraph counts must match exactly or it silently misaligns" risk as the neijing zip, but caught loudly here (`ValueError` at build time naming the mismatched counts) rather than failing silently. `entry["bilingual"]` gates which branch `journal_entry.html` renders — plain `entry.html` for Chinese-only entries (the common case, e.g. 厥孚复利, 定投的威力), `.bilingual-pair`/`.bilingual-zh`/`.bilingual-en` pairs for entries with a translation (e.g. 如何解卦). `body_text` (used by `search()`) always stays Chinese-only regardless of bilingual status.
